/** Shared chart theme, in src/viz — today's callers are the Internals interp
 *  drivers (and the 2D chrome charts), but nothing here reaches back into
 *  chrome/ or scene/, so a future Seer front-end can pull it in on its own.
 *  Before this module every driver hand-rolled its own grid colors,
