/** Nebulai-only e2e plumbing: boot the app on a given gpu rung, collect
 *  console errors from the very first request, and wait for the deep-linked
 *  view. All state waits go through `window.__store` / `window.__perf`
 *  (exposed by main.ts for exactly this purpose) — never through timing
 *  guesses.
 *
 ...
 *  for exactly that reason instead of importing this one. Only `rungOf` /
